...
void FileSystem::insertContent(const string& filename, const string &content){
    ...
    int* timestamp_idx_ptr = timestamp_heap_indices.get(fileptr);
        if (timestamp_idx_ptr) {
            ...
            //"I know the element at index *timestamp_idx_ptr has just changed its value.don't need to search for it.
            // Just go directly to that spot and run  heapifyUp and heapifyDown logic to fix its position
        }
...
void FileSystem::snapshot(const string & filename, const string &message){
    ...
    // Perform the snapshot: set the timestamp and the message.
...
void FileSystem::rollBack(const string& filename, const string& versionid){
    ...
    if(versionid.empty() ){
        ...
        if(is_valid_number){
            ...
            cout << "Error: Invalid ID '" << versionid << "'. Please provide a whole number." << endl;
        }
    }
}
void FileSystem::history(const string& filename){
    ...
    for(VersionNode* i : snapshottednodes){
        ...
        // so remove this character (time_str.pop_back();) for cleaner formatting.
...
void FileSystem::recentFiles(int count) {
    ...
    //  Extract, print, and store in a temporary vector.
    ...
    for (int i = 0; (i < num_to_show)  && (recent_files_heap.pop(currfile)); i++) {
        // pop operation guarantees that file exists in heap only then would the loop run
    ...
    // Re-insert the files to restore the heap.
