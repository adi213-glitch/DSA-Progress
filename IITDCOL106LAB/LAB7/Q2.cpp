...
int func(vector<int> &product_ids){
    int n = product_ids.size();
    if(n==0) return 0;
    MyHashMap m (n);
    int i=0,j=0;
    int mxsize=0;
    while(j<n){
        if(m.get(product_ids[j])==NULL){
            m.insert(product_ids[j],1);
            
            j++;
        }
        else{
            m.remove(product_ids[i]);
            i++;
        }
        if (m.size() >mxsize ) mxsize= m.size();
    }
    return mxsize;
