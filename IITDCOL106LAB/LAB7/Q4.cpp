...
bool func(vector<int> &signal_frequencies){
    int n = signal_frequencies.size();
    
    MyHashMap m(n);
    
    for(int i=0; i<n;i++){
        
        if(m.get(2*signal_frequencies[i])!=NULL  || (signal_frequencies[i]%2==0 && m.get(signal_frequencies[i]/2)!=NULL)){
            return 1;   
        }
        m.insert(signal_frequencies[i],1);
